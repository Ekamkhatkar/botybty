...
const { checkIdentifier } = require('./Events/Client/ready.js');
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (!client.config.logchannel) {
    console.warn(`Warning: No log channel ID specified in config.js`);
    return;
  }
  ...
  if (!channel) {
    console.warn(`Warning: Could not find log channel with ID ${client.config.logchannel}. Make sure the channel exists and the bot has access to it.`);
    return;
  }
  ...
  try {
    ...
    console.error(`Error sending command log to channel: ${error.message}`);
  }
});
